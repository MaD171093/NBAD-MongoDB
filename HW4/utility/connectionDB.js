...
db.once('open', function(){
  ...
  connection.find(async function (err, connections) {
    if (err) return console.error(err);
    // console.log(connections);
    connectionList = await connections;

    connections.forEach(createTopicArray);
...
async function createTopicArray(item){
  await topicArray.push(item.connectionTopic);
};
